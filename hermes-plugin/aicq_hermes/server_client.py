...
class AicqServerClient:
    ...
    async def list_friends(self) -> list:
        session = await self._get_session()
        async with session.get(f"{self.api_base}/friends", headers=self._auth_headers()) as resp:
            data = await resp.json()
            return data.get("friends", [])

    async def send_friend_request(self, to_id: str) -> dict:
        session = await self._get_session()
        async with session.post(f"{self.api_base}/friends/request", json={
            "to_id": to_id,
    ...
    async def list_friend_requests(self) -> list:
        session = await self._get_session()
        async with session.get(f"{self.api_base}/friends/requests", headers=self._auth_headers()) as resp:
            data = await resp.json()
    ...
    async def accept_friend_request(self, request_id: str) -> dict:
        ...
        async with session.post(
            f"{self.api_base}/friends/requests/{request_id}/accept",
    ...
    async def add_friend_by_number(self, aicq_number: str) -> dict:
        ...
        async with session.get(
            f"{self.api_base}/accounts/resolve?number={aicq_number}",
            headers=self._auth_headers(),
        ) as resp:
            data = await resp.json()
    ...
    async def send_chat_message(self, to_id: str, content: str, msg_type: str = "text") -> dict:
        session = await self._get_session()
        async with session.post(f"{self.api_base}/chat/messages", json={
            "to": to_id,
    ...
    async def get_conversation(self, friend_id: str, limit: int = 50) -> dict:
        session = await self._get_session()
        async with session.get(
            f"{self.api_base}/chat/conversation/{friend_id}?limit={limit}",
            headers=self._auth_headers(),
        ) as resp:
            return await resp.json()
    ...
    async def mark_read(self, friend_id: str) -> dict:
        session = await self._get_session()
        async with session.post(f"{self.api_base}/chat/mark-read", json={
            "friend_id": friend_id,
        }, headers=self._auth_headers()) as resp:
            return await resp.json()
